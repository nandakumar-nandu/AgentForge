...
AgentForge contains advanced telemetry features designed to keep developers fully in control of their experimental and operational spend.

1. **How to Read the Analytics Dashboard**:
   - **Monthly Spend**: Displays the estimated USD cost of all tokens processed since the first day of the current calendar month.
   - **Cost Allocation Chart**: A Recharts bar visualization illustrating the total cost generated by each individual agent. This is invaluable to identify which assistant is generating the bulk of your invoice bills.
   - **Token Consumption Trend**: A dual line chart mapping input (prompt) vs output (completion) tokens. High prompt counts compared to completion counts indicate that your system prompts or dialogue history arrays are too large.

2. **Identifying Cost Anomalies & "Expensive" Agents**:
   - Check the **Telemetry Log Table** regularly. Sort or filter by **Agent** or **Model** to pinpoint individual execution runs that consumed outsized resources.
   - For example, if a "qa" agent model frequently generates logs costing $>0.05$ USD per reply, audit its system prompt and consider pruning historical contexts.

3. **Setting Budgets & Prompts Optimization**:
   - **Use Cheaper Models**: For basic sorting or formatting tasks, use templates pinned to standard models before routing requests to high-cost reasoning engines.
   - **Prune History**: Interactive dialogue buffers should maintain a rolling window of the last 5-10 messages, rather than passing the entire conversation thread in every request.
